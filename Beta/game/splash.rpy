...
init -100 python:
    ...
    for archive in ['audio','images','scripts','fonts']:
        if archive not in config.archives:
            ...
            renpy.error("Los archivos de DDLC no han sido encontrados en la carpeta /game. Compruebe la instalación e inténtelo otra vez.")
...
init python:
    ...
    splash_message = _("Este juego está hecho por fans, por lo que no es oficial. No afiliado con el Team Salvato.")
...
label splashscreen:
    ...
    if not persistent.has_launched_before:
        ...
        with Dissolve(1.0)
        pause 1.0
        "[config.name] es un mod de Doki Doki Literature Club hecho por fans que no está afiliado con el Team Salvato."
        "Ha sido creado para ser jugado solo tras completar el juego oficial por lo que contiene spoilers del mismo."
        "Los archivos del juego original se pueden descargar gratuitamente en: http://ddlc.moe"
        $ narrator(
            "Al jugar [config.name] aceptas haber completado Doki Doki Literature Club y cualquier spoiler que este mod pueda contener.",
            interact=False
        )
        $ renpy.display_menu(items=[ ("Estoy de acuerdo.", True)], screen="choice_centred")
        scene tos_b
        with Dissolve(1)
        pause 1.0
