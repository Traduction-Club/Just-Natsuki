...
init 0 python:
    ...
    TOPIC_TYPE_FAREWELL = "DESPEDIDA"
    TOPIC_TYPE_GREETING = "SALUDO"
    TOPIC_TYPE_NORMAL = "NORMAL"
    TOPIC_TYPE_ADMISSION = "CONFESIÓN"
    TOPIC_TYPE_COMPLIMENT = "CUMPLIDO"
    TOPIC_TYPE_APOLOGY = "DISCULPA"
    TOPIC_TYPE_EVENT = "EVENTO"
    ...
    class Topic(object):
        ...
        def __init__(
            ...
            if not isinstance(persistent_db, dict):
                raise Exception("La base de datos persistente proporcionada no es de tipo dict")
            ...
            if not renpy.has_label(label):
                raise Exception("La etiqueta {0} no existe.".format(label))

            #Validate the affinity range prior to it
            if not jn_affinity._isAffRangeValid(affinity_range):
                raise Exception("El rango de afinidad: {0} no es válido.".format(affinity_range))
    ...
    def registerTopic(Topic, topic_group=TOPIC_TYPE_NORMAL):
        ...
        if local_map is None:
            raise Exception("El tipo de tema '{0}' no es un mapa registrado.")

        elif not isinstance(local_map, dict):
            raise Exception("El mapa temático para el tipo '{0}' no es del tipo dict.")
    ...
    def jn_get_holiday_for_date(input_date=None):
        ...
        if input_date is None:
            ...
            raise TypeError("input_date para la comprobación de vacaciones el dato debe ser del tipo fecha; su tipo es {0}".format(type(input_date)))
...
init -990 python in jn_globals:
    ...
    DEFAULT_TALK_FLAVOR_TEXT_LOVE_ENAMORED = [
        "¿Qué pasa,{w=0.1} [player]?",
        "¿Qué pasa por esa cabeza tuya,{w=0.1} [player]?",
        "¿Ha pasado algo nuevo,{w=0.1} [player]?",
        "¿Quieres charlar un rato?{w=0.2} Jejeje.",
        "¡Me encantan estas charlas!",
        "¡Siempre estoy encantada de hablar contigo,{w=0.1} [player]!",
        "¡[player]!{w=0.2} ¿Qué pasa?",
        "¡[player]!{w=0.2} ¿Qué tienes en mente?",
        "¡Ooh!{w=0.2} ¿Sobre qué quieres hablar ahora?",
        "¡Soy todo oídos,{w=0.1} [player]~!",
        "¡Siempre estoy lista para hablar de lo que sea contigo,{w=0.1} [player]!"
    ]
    ...
    DEFAULT_TALK_FLAVOR_TEXT_AFFECTIONATE_NORMAL = [
        ...
        "¿Qué tienes en mente?",
        "¿Qué quieres?",
        "¿Algo ronda por tu mente?",
        "¿Oh?{w=0.2} ¿Quieres hablar conmigo?",
        "¿Eh?{w=0.2} ¿Qué pasa?",
        "¿Tienes algo que compartirme?",
        "¡Ey!{w=0.2} ¿Qué querías?",
    ...
    DEFAULT_ALPHABETICAL_ALLOW_VALUES = "ABCDEFGHIJKLMNÑOPQRSTUVWXYZabcdefghijklmnñopqrstuvwxyz-' "
