init python in jn_threading:
    ...
    def fire_and_forget(function, args=()):
        ...
        if not callable(function):
            raise TypeError("la función dada no es un objeto invocable")
            return

        if not isinstance(args, tuple) and not isinstance(args, list):
            raise TypeError("los argumentos deben darse en formato de tupla o lista")
            return
        ...
        try:
            ...
            jn_utils.log(message="Error al iniciar el hilo thread para la función {0} con args {1}".format(function, args), logseverity=jn_utils.SEVERITY_ERR)
