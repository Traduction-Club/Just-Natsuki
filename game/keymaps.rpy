init -50 python:
    def jn_register_label_keymap(keymap_name, keymap_label, *keys):
        ...
        if not renpy.has_label(keymap_label):
            jn_utils.log("[ERROR]: Se ha intentado registrar una tecla para una etiqueta que no existe.", jn_utils.SEVERITY_WARN)
    ...
    def jn_register_keymap(keymap_name, keymap_action, *keys):
        ...
        if keymap_name in config.keymap:
            jn_utils.log("ERROR: Se ha intentado registrar una tecla con un nombre existente. Ignorando.", jn_utils.SEVERITY_WARN);
            return

        #Validate the keymap is indeed callable
        if not callable(keymap_action):
            jn_utils.log("ERROR: La acción de la tecla proporcionada no es invocable. Ignorando.", jn_utils.SEVERITY_WARN)
            return
